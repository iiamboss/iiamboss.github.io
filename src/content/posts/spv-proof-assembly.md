...
조상 그래프는 엄밀히는 DAG다. 같은 조상이 서로 다른 경로로 다시 나타날 수 있다.
그래도 같은 깊이끼리는 서로 기다릴 필요가 없다. 각 조상의 조회는 txid 하나로 끝나는
독립 호출이라 형제의 결과에 의존하지 않는다. 병렬화의 근거는 그래프 모양이 아니라
그 독립성이다. 순차 재귀는 이 사실을 그냥 버리고 있었다. 그래서 DFS 재귀를 BFS 레벨
배치로 바꿨다.
...
직렬 대기 횟수가 조상 수(N)에서 그래프 깊이로 줄었다. 호출 총량이 준 건 아니다.
83입력짜리도 깊이는 한 자릿수였다.
...
깊은 그래프에는 여전히 약하다. 레벨 수만큼은 직렬이라 깊이가 수십으로 늘면 다시
